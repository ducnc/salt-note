...
#neutron-api_nova.conf:
# file.managed:
#  - name: /etc/nova/nova.conf
#  - source: salt://neutron/api/files/nova.conf
#  - template: jinja
#  - require:
#     - pkg: pkgs_neutron-api-server
...
neutron_command:
 cmd.run:
  - name: su -s /bin/sh -c "neutron-db-manage --config-file /etc/neutron/neutron.conf --config-file /etc/neutron/plugins/ml2/ml2_conf.ini upgrade juno" neutron
  - require:
     - file: neutron.conf
